...
$sql = "SELECT * FROM `orders` JOIN `customer` ON `customer`.`id`=`orders`.`customer_id` JOIN `payment` ON `payment`.`order_id`=`orders`.`order_id` WHERE `orders`.`delivery_status`='processing' GROUP BY `orders`.`order_id` ORDER BY `orders`.`id` DESC ";
$stmt = $conn->query($sql);
?>
<?php /*
