...
$sql = "SELECT * FROM `orders` JOIN `customer` ON `customer`.`id`=`orders`.`customer_id` JOIN `payment` ON `payment`.`order_id`=`orders`.`order_id` WHERE `orders`.`delivery_status`='delivered' GROUP BY `orders`.`order_id` ORDER BY `orders`.`id` DESC";
$stmt = $conn->query($sql);
?>
<?php /*
...
if ($id == '1') {
    $sql = "SELECT * FROM `order`JOIN `user_details` ON `order`.`user_id`=`user_details`.`user_id` where `payment_date`='$TodayDate';";
}else{
$sql = "SELECT * FROM `order`JOIN `user_details` ON `order`.`user_id`=`user_details`.`user_id`;";
}
                            ...
                            <table id="example-table" class="table table-striped table-bordered table-hover table-green">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 78px;">grand total</th>
                                        <th style="width: 193px;">orderDate</th>
                                        <th style="width: 193px;">delivery_status</th>
                                        <th style="width: 78px;">view</th>
                                    </tr>
                                </thead>
                                <tbody>
